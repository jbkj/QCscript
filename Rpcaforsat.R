...
PCA1Corrected <- pc$PCA1[-c(which(pc$PCA1>PCA1_high),which(pc$PCA1<PCA1_low))]
PCA2Corrected <- pc$PCA2[-c(which(pc$PCA2>PCA2_high),which(pc$PCA2<PCA2_low))]


png("PCA.png",width = 960, height=960)
#plot(pc[c(which( (pc$PCA1>PCA1_high && pc$PCA1<PCA1_low) && (pc$PCA2>PCA2_high & pc$PCA2<PCA2_low) ) ), 1:2] )
plot(PCA1Corrected,PCA2Corrected)
dev.off()    

out <- row.names( pc[ c(which(pc$PCA1>PCA1_high),which(pc$PCA1<PCA1_low), which(pc$PCA2>PCA2_high),which(pc$PCA2<PCA2_low)),])
